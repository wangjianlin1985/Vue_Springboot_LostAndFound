...
export default {
  data() {
	...
	xiangpian : false,
      ...
      rules: {
          fabuxuehao: [
                { required: true, message: '发布学号不能为空', trigger: 'blur' },
          ],
          mima: [
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `baoguanyuan/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.baoguanyuanCrossAddOrUpdateFlag = false;
						 this.parent.search();
